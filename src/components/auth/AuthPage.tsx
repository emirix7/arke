...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (mode === 'register' && !acceptedPolicy) { setError('Kullanım koşullarını kabul etmelisiniz.'); return }
      ...
      if (mode === 'register') {
        ...
        if (data.user) {
          await (supabase.from('profiles') as any).insert({
            ...
            display_name: username.trim(), status: 'online',
            allow_messages_from: 'everyone', updated_at: new Date().toISOString(),
          })
          setUser(data.user); await fetchProfile(data.user.id)
        }
      } else {
        const fakeEmail = `${username.toLowerCase().trim()}@arke.app`
        const { data, error: signInError } = await supabase.auth.signInWithPassword({ email: fakeEmail, password })
        if (signInError) { setError('Kullanıcı adı veya şifre hatalı.'); setLoading(false); return }
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#0d0d14' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <div className="flex flex-col items-center mb-8">
          <img src="/arke-logo.png" alt="Arke" style={{ width: 64, height: 64, borderRadius: '50%', objectFit: 'cover', marginBottom: 16 }} />
        ...
        <div className="rounded-2xl p-6" style={{ background: '#10101c', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-xs font-semibold mb-2 tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.35)' }}>Kullanıcı Adı</label>
              <input type="text" value={username} onChange={e => setUsername(e.target.value)}
                placeholder="kullanici_adin" minLength={3} maxLength={24} pattern="[a-zA-Z0-9._]+" required
            ...
            <div>
              <label className="block text-xs font-semibold mb-2 tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.35)' }}>Şifre</label>
            ...
            {mode === 'register' && (
              <label className="flex items-start gap-2.5 cursor-pointer">
                <div onClick={() => setAcceptedPolicy(p => !p)}
                  className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0 mt-0.5 transition-all"
                  style={{ background: acceptedPolicy ? '#c044ff' : 'rgba(255,255,255,0.08)', border: `1px solid ${acceptedPolicy ? '#c044ff' : 'rgba(255,255,255,0.15)'}` }}>
                  {acceptedPolicy && <span style={{ color: 'white', fontSize: 10, fontWeight: 700 }}>✓</span>}
                </div>
                <span className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.45)' }}>
                  <button type="button" onClick={() => setShowPolicy(true)}
                    style={{ color: '#c044ff', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit', fontSize: 'inherit' }}>
            ...
            <button type="submit" disabled={loading}
              ...
              style={{ background: loading ? 'rgba(192,68,255,0.3)' : 'linear-gradient(135deg, #c044ff, #00d4ff)', cursor: loading ? 'not-allowed' : 'pointer' }}>
              {loading ? (
